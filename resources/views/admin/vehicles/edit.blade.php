    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.vehicles.update", [$vehicle->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('customer') ? 'is-invalid' : '' }}" name="customer_id" id="customer_id" required>
                    @foreach($customers as $id => $entry)
                        <option value="{{ $id }}" {{ (old('customer_id') ? old('customer_id') : $vehicle->customer->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('vehicle_color') ? 'is-invalid' : '' }}" type="text" name="vehicle_color" id="vehicle_color" value="{{ old('vehicle_color', $vehicle->vehicle_color) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('plate_chars') ? 'is-invalid' : '' }}" type="text" name="plate_chars" id="plate_chars" value="{{ old('plate_chars', $vehicle->plate_chars) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('vehicle_brand') ? 'is-invalid' : '' }}" type="text" name="vehicle_brand" id="vehicle_brand" value="{{ old('vehicle_brand', $vehicle->vehicle_brand) }}" required>
