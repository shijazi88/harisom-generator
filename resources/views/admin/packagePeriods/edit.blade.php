...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.packagePeriod.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.package-periods.update", [$packagePeriod->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('package') ? 'is-invalid' : '' }}" name="package_id" id="package_id" required>
                    @foreach($packages as $id => $entry)
                        <option value="{{ $id }}" {{ (old('package_id') ? old('package_id') : $packagePeriod->package->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('discount') ? 'is-invalid' : '' }}" type="number" name="discount" id="discount" value="{{ old('discount', $packagePeriod->discount) }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('period_value') ? 'is-invalid' : '' }}" type="number" name="period_value" id="period_value" value="{{ old('period_value', $packagePeriod->period_value) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('period_label_en') ? 'is-invalid' : '' }}" type="text" name="period_label_en" id="period_label_en" value="{{ old('period_label_en', $packagePeriod->period_label_en) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('period_label_ar') ? 'is-invalid' : '' }}" type="text" name="period_label_ar" id="period_label_ar" value="{{ old('period_label_ar', $packagePeriod->period_label_ar) }}" required>
