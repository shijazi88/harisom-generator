    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.packages.update", [$package->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{{ old('description', $package->description) }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('discount') ? 'is-invalid' : '' }}" type="number" name="discount" id="discount" value="{{ old('discount', $package->discount) }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('package_type') ? 'is-invalid' : '' }}" name="package_type" id="package_type" required>
                    ...
                    @foreach(App\Models\Package::PACKAGE_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('package_type', $package->package_type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('package_period') ? 'is-invalid' : '' }}" type="number" name="package_period" id="package_period" value="{{ old('package_period', $package->package_period) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('package_period_unit') ? 'is-invalid' : '' }}" type="text" name="package_period_unit" id="package_period_unit" value="{{ old('package_period_unit', $package->package_period_unit) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status" required>
                    ...
                    @foreach(App\Models\Package::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $package->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
